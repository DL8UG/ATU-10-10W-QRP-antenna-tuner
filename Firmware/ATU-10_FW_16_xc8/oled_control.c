...
// Doubles the 4 bits of a nibble vertically: bit n -> bits 2n and 2n+1
static char dbl_nibble(char n) {
   char i, r = 0;
   for(i = 0; i < 4; i++)
      if(BIT(n, i)) r |= 3 << (2 * i);
   return r;
...
// Battery bar segment with u (0..8) pixels filled from the top
static char bar(char u) {
   if(u == 0) return 0;
   return 256 - (128 >> (u - 1));
}
...
void oled_wr_str (char page, char col, const char *str, char leng ) {  //
  ...
  for (i = 0; i < leng; i++) { // write string
     ...
     for (h = 0; h <= 4; h++) {  // write letter
      w1 = font_5x8[g*5+h];
      if(page != 2)
         w2 = dbl_nibble(w1 & 0x0F);
      else
         w2 = dbl_nibble(w1 & 0x07) << 2;
      Soft_I2C_Write(w2);
      Soft_I2C_Write(w2);
      }
     Soft_I2C_Write (0);
